...
var ViewModel = function() {
    ...
    this.searchedLoc = ko.observable("");

    listedLocations.forEach(function(locationItem) {
        self.markers.push( new LocationReference(locationItem));
    });
    
    this.newMarkers = ko.computed(function() {
        var filter = self.searchedLoc().toLowerCase();
        if (!filter) {
            ...
            return ko.utils.arrayFilter(self.markers(), function(locationItem) {
                var term = locationItem.title.toLowerCase();
                var newTerm = (term.search(filter) >= 0);
                locationItem.isVisible(newTerm);
                return newTerm;
            });
        }
    }, self);
